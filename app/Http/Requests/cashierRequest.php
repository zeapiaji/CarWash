...
class cashierRequest extends FormRequest
{
    ...
    {
        return [
            'name' => 'required|max:25|min:5|unique:users,name',
            'email' => 'required|unique:users,email|email:rfc,dns|min:5|max:20',
            'phone' => 'required|unique:users,phone|min:11|max:13',
            'birth' => 'required',
            'gender' => 'required',
